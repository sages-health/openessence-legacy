...
Ext.namespace("OE.input.datasource.search");
 ...
 */
OE.input.datasource.search.form = function (configuration) {

    var sm = new Ext.grid.RowSelectionModel({singleSelect: OE.util.getBooleanValue(configuration.singleSelect, false)});

    var gridPanel = OE.datasource.grid.init({
        title: messagesBundle[configuration.dataSource + '.results'] || messagesBundle['input.datasource.default.results'],
        selectionModel: sm,
    ...
    return new Ext.Panel({
        ...
        items: [new OE.report.ReportForm(Ext.apply(configuration, {
            detailsNoGroupByCallback: function (parameters) {
                gridPanel.load({params: Ext.apply(parameters, {firstrecord: 0, pagesize: Ext.num(dimensionsBundle['page.size'], 100)})});
            }
        })), resultsFormPanel]
    });
};
