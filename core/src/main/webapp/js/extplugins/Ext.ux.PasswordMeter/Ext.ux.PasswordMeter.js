...
Ext
		.extend(
				...
				{
					/**
					 * @cfg {String} inputType The type attribute for input
					 *      fields -- e.g. text, password (defaults to
					 *      "password").
					 */
					inputType : 'text',
					// private
					onRender : function(ct, position) {
						Ext.ux.PasswordMeter.superclass.onRender.call(this, ct,
								position);

						// var elp = this.el
						// .findParent('.x-form-element', 5, true);
						this.objMeter = ct.createChild({
							tag : "div",
							'class' : "strengthMeter"
						});

						this.objMeter.setWidth(this.el.getWidth());
					...
					updateMeter : function(e) {
						...
						this.updateMeterVal(this, p);
					},
					updateMeterVal : function(me, val) {
						var maxWidth = me.objMeter.getWidth() - 2;

						var nScore = me.calcStrength(val);
						...
						me.scoreBar.setWidth(scoreWidth, true);
					},
					/**
					...
					calcStrength : function(p) {
						...
						if (p.length > 0 && p.length <= 4) { // length 4 or
							// less
							intScore += p.length;
						} else if (p.length >= 5 && p.length <= 7) { // length
							// between
							// 5 and
							// 7
							intScore += 6;
						} else if (p.length >= 8 && p.length <= 15) { // length
							// between
							// 8 and
							// 15
							intScore += 12;
						...
						if (p.match(/[a-z]/)) { // [verified] at least one lower
							// case letter
							intScore += 1;
						}
