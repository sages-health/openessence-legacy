...
public abstract class FieldFilter implements SqlGeneratingFilter {

    private final String filterId;

    public FieldFilter(String filterId) {
        this.filterId = filterId;
    }

    public abstract List<Object> getArguments();

    public abstract String getSqlSnippet(String colName);

    public String getSqlSnippet(JdbcOeDataSource ds) {
        DimensionBean bean = ds.getBean(filterId);
        if (bean != null) {
            return getSqlSnippet(bean.getSqlCol());
        } else {
            return null;
        }
    }
    ...
    public void checkFilter(JdbcOeDataSource ds) throws OeDataSourceException {
        ...
        if (bean == null) {
            throw new OeDataSourceException(
                    "Filter on unrecognized dimension "
                    + getFilterId());
        }
    }
}
